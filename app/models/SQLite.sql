...
-- SELECT DispositivosEquipamentos.serial, TipoEquipamentos.nome, PontoAtendimentos.descricao FROM DispositivosEquipamentos JOIN TipoEquipamentos on DispositivosEquipamentos.idTipo=TipoEquipamentos.id JOIN Computadores on DispositivosEquipamentos.id=Computadores.idDispositosEquipamento JOIN PontoAtendimentos on Computadores.idPontoAtendimento=PontoAtendimentos.id WHERE DispositivosEquipamentos.idSite=1 AND DispositivosEquipamentos.id=1
-- SELECT PontoAtendimentos.descricao FROM PontoAtendimentos WHERE PontoAtendimentos.idSite=1
SELECT DispositivosEquipamentos.serial, PontoAtendimentos.descricao FROM DispositivosEquipamentos JOIN Computadores ON Computadores.idDispositosEquipamento=DispositivosEquipamentos.id JOIN PontoAtendimentos on Computadores.idPontoAtendimento=PontoAtendimentos.id WHERE DispositivosEquipamentos.idSite=1 AND PontoAtendimentos.descricao='TA-BA-F1-P2'
